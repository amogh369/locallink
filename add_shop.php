...
<head>
  ...
  <style>
    ...
    .map-hint { font-size:0.82rem; color:var(--muted); margin-top:6px; }
  </style>
</head>
<body>
...
<div class="page-wrapper">
  <div class="page-hero" style="padding-bottom:80px;">
    <div style="position:relative;z-index:2;">
      ...
      <div class="hero-title">Add Your <span>Shop</span> 🏪</div>
      <div class="hero-subtitle">Register your shop and start receiving orders!</div>
    </div>
    ...
    <div class="card-ll" style="padding:24px;max-width:600px;margin:0 auto;">
      ...
      <form onsubmit="addShop(event)">
        <div class="form-group-ll">
          ...
          <input class="form-control-ll" type="text" id="shop-name" placeholder="e.g. Fresh Mart Grocery" required /></div>
        </div>
        <div class="form-group-ll">
        ...
        <div class="form-group-ll">
          ...
          <textarea class="form-control-ll" id="shop-desc" rows="3" placeholder="Describe your shop, specialties, etc."></textarea>
        </div>
        <div class="form-group-ll">
          <label class="form-label-ll">Phone Number</label>
          <div class="input-group-ll"><span class="input-icon">📞</span>
          <input class="form-control-ll" type="tel" id="shop-phone" placeholder="Shop contact number" maxlength="10" /></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group-ll">
            <label class="form-label-ll">Opening Time</label>
            <input class="form-control-ll" type="time" id="shop-open" value="08:00" />
          </div>
          <div class="form-group-ll">
            <label class="form-label-ll">Closing Time</label>
            <input class="form-control-ll" type="time" id="shop-close" value="22:00" />
          </div>
        </div>

        <!-- MAP PICKER -->
        <div class="form-group-ll">
          <label class="form-label-ll">📍 Shop Location (click on map to set) *</label>
          <div id="pick-map"></div>
          <div class="map-hint">🖱 Click on the map to pin your exact shop location</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px;">
            <div>
              <label class="form-label-ll">Latitude</label>
              <input class="form-control-ll" type="text" id="shop-lat" placeholder="Auto-filled from map" readonly />
            </div>
            <div>
              <label class="form-label-ll">Longitude</label>
              <input class="form-control-ll" type="text" id="shop-lng" placeholder="Auto-filled from map" readonly />
            </div>
          </div>
        ...
        <div class="form-group-ll">
          <label class="form-label-ll">Address (text)</label>
          <div class="input-group-ll"><span class="input-icon">📍</span>
          <input class="form-control-ll" type="text" id="shop-address" placeholder="Full address of your shop" /></div>
        </div>

        <button type="submit" class="btn-ll btn-primary-ll btn-full" id="add-btn">
          🏪 Register Shop
        </button>
      </form>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  pickMap.setView([12.9716,77.5946], 13);
  pickMap.on('click', e => setPin(e.latlng.lat, e.latlng.lng));
  useMyLocation();
});

function setPin(lat, lng) {
  pickedLat = lat.toFixed(7); pickedLng = lng.toFixed(7);
  ...
  pickMarker = L.marker([lat,lng]).addTo(pickMap).bindPopup('<b>Your Shop Location</b>').openPopup();
  // Reverse geocode
  fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`)
    .then(r=>r.json()).then(d=>{
      const addr = d.display_name || '';
      document.getElementById('shop-address').value = addr.split(',').slice(0,4).join(',');
    }).catch(()=>{});
}
...
async function addShop(e) {
  e.preventDefault();
  if (!pickedLat || !pickedLng) { showAlert('Please select shop location on the map.','danger'); return; }
  const btn = document.getElementById('add-btn');
  btn.innerHTML='⏳ Registering...'; btn.disabled=true;
  const res = await fetch('api/add_shop.php', {
  ...
  if(data.success){ showAlert('Shop registered successfully! 🎉','success'); setTimeout(()=>location.href='shop_dashboard.php',1500); }
  else{ showAlert(data.message||'Failed to add shop.','danger'); btn.innerHTML='🏪 Register Shop'; btn.disabled=false; }
}
