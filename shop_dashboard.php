...
$ms = $pdo->prepare("SELECT * FROM shops WHERE owner_id=? ORDER BY created_at DESC");
...
$po = $pdo->prepare("
    ...
    WHERE s.owner_id=? AND o.status='pending' ORDER BY o.created_at DESC
");
$po->execute([$userId]); $pendingOrders=$po->fetchAll();

$rev = $pdo->prepare("SELECT COALESCE(SUM(o.total_amount),0) AS r FROM orders o JOIN shops s ON o.shop_id=s.shop_id WHERE s.owner_id=? AND o.status='delivered'");
$rev->execute([$userId]); $totalRevenue=$rev->fetchColumn();

$cnt = $pdo->prepare("SELECT COUNT(*) FROM orders o JOIN shops s ON o.shop_id=s.shop_id WHERE s.owner_id=?");
$cnt->execute([$userId]); $totalOrders=(int)$cnt->fetchColumn();
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="page-wrapper">
  <div class="page-hero" style="padding-bottom:80px;">
    <div style="position:relative;z-index:2;">
      ...
      <div class="hero-subtitle">Manage your shops, products &amp; orders</div>
    </div>
  ...
  <div class="content-body">
    ...
    <?php if(empty($pendingOrders)):?>
      <div class="card-ll" style="padding:24px;text-align:center;margin-bottom:24px;"><div style="font-size:2.5rem;margin-bottom:8px;">🎉</div><div style="font-weight:700;color:var(--muted);">No pending orders!</div></div>
    <?php else:?>
    <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px;">
      ...
      <div class="card-ll" style="padding:16px;">
        ...
        <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button class="btn-ll btn-sm-ll" style="background:rgba(17,153,142,0.1);color:var(--success);" onclick="upd(<?=$o['order_id']?>,'delivered')">🎉 Delivered</button>
    ...
    <?php if(empty($myShops)):?>
      <div class="empty-state"><div class="empty-icon">🏗</div><h3>No shops yet</h3><p>Add your first shop!</p><a href="add_shop.php" class="btn-ll btn-primary-ll" style="margin-top:16px;">➕ Add Shop</a></div>
    <?php else:?>
...
<script>
async function upd(id,status){const res=await fetch('api/update_order.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_id:id,status})});const data=await res.json();if(data.success){showToast('Updated!','Status: '+status.replace(/_/g,' '),'success');setTimeout(()=>location.reload(),1500);}else showToast('Error',data.message||'Failed','error');}
